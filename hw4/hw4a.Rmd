...
From the `admissions` table, obtain `admittime` and `dischtime` times for each patient so we can calculate length of hospital stay. We can also obtain the length of each ICU stay from the `icustays` table.
...
## Export to RDS for use in visualization and analysis
