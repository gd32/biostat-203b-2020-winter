...
The data is imported in wide format (as it is longitudinal). I will use Dr. Zhou's framework to convert the imported CSVs into long format:
...
confirmed = confirmed_raw %>%
  pivot_longer(-(`Province/State`:Long), 
               names_to = "time", 
               values_to = "confirmed") %>%
  mutate(time = (mdy_hm(time))) # convert string to date-time
...
ncov_tbl %>%
  filter(`Country/Region` %in% c("Mainland China", "Macau", "Hong Kong", "Taiwan")) %>%
  filter(date(time) == plotdate) %>%
  group_by(`Province/State`) %>%  
  top_n(1, time) %>% # take the latest count on that date
...
stock <- getSymbols("^HSI", # S&P 500 (^GSPC), Dow Jones (^DJI), NASDAQ (^IXIC), Russell 2000 (^RUT), FTSE 100 (^FTSE), Nikkei 225 (^N225), HANG SENG INDEX (^HSI)
                    src = "yahoo", 
                    auto.assign = FALSE, 
                    from = min(ncov_tbl$time),
                    to = max(ncov_tbl$time)) %>% as_tibble(rownames = NA)
stock
