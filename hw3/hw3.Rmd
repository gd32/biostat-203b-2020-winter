...
JHU's CSSE maintains a data repository <https://github.com/CSSEGISandData/COVID-19> tracking all COVID cases of the most recent outbreak. Data is broken down into `Confirmed`, `Deaths`, and `Recovered` cases and updated daily. 
...
covid %>% filter(`Country/Region` %in% c("Mainland China", 
                                         ...
                                         "Hong Kong", 
                                         "Taiwan")) %>% 
  mutate(isHubei = ifelse(`Province/State` == "Hubei", "Hubei", "Others")) %>%
  ggplot() + 
  geom_col(aes(date, confirmed, fill = isHubei)) +
  labs(fill = "Province") +
  xlab("Date") +
  ylab("Cases") +
...
## Write data to file for use in Shiny app
...
## Mapping China provinces using `nCov2019`
...
These are the provinces available from the JHU data.
...
## Create a new variable `NAME_ENG` converting Chinese placenames to English
...
```{r}
write_csv(chn_prov, "./covid_map/provs.csv")
```
