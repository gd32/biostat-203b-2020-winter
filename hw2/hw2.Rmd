...
icu_admit_pts %>% select(contains('subject_id'))
...
```{r}
final = final_c %>% 
        mutate(died_within_30 = 
               case_when(is.na(death_time) ~ F, 
...
final_selected %>% 
  ...
  ylab("Proportion") +
  ggtitle("Death Status of Patients in First ICU Stay")
```
